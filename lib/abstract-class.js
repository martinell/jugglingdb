...
AbstractClass.prototype._initProperties = function (data, applySetters) {
    ...
    if (typeof ctor.parentSchema !== 'undefined') {
        _.extend(properties, ctor.schema.definitions[ctor.parentSchema.modelName].properties);    
    }
...
AbstractClass.find = function find(id, cb) {
    ...
    this.schema.adapter.find(this.modelName, id, function (err, data) {
        ...
        if (data) {
            ...
            obj = new this();
            retrieveParentData(id, data, obj, cb);
        } else {
            cb(err, obj);    
        }
    }.bind(this));
...
function findParentInstance(id, cb){
    this.parentSchema.find(id, cb);
}

function retrieveParentData(id, childData, obj, cb) {
    var obj = obj || new this();
    if (typeof this.parentSchema !== 'undefined') {
        this.parentSchema.find(id, function (err, parentModel) {
            if (parentModel) {
                obj.__proto__.__proto__ = parentModel.__proto__;
                var mergedData = {};
                _.extend(mergedData, parentModel.__data, childData);
                obj._initProperties(mergedData, false);
            }
            cb(err, obj);
        });
    } else { 
        obj._initProperties(childData, false);
        cb(null, obj);
    }
};

function retrieveAllParentData(childArray, cb) {
    var i = 0;
    var self = this;
    async.forEach(childArray, function (childData, nextcb) {
        var obj = new self();
        retrieveParentData.call(self, childData.id, childData, obj, function (error, obj) {
            if(obj){
                childArray[i]=obj;
                i++;
            }
...
AbstractClass.all = function all(params, cb) {
    ...
    this.schema.adapter.all(this.modelName, params, function (err, data) {
        if (data && data.map) {
            retrieveAllParentData.call(constr, data, function (error, data) {
                ...
                inst.trigger('update', function (updateDone) {
                    inst._adapter().save(modelName, inst.constructor._forDB(data), function (err) {
                        ...
                        if (err) {
                            ...
                            if (typeof inst.constructor.parentSchema !== 'undefined') {
                                findParentInstance(inst.id, function (error, parentInstance) {
                                    if(parentInstance){
                                        parentInstance.save(options, function (error, parentInstance) {
                                            retrieveParentData(inst.id, data, inst, function (error) {
                                                callUpdateDone(error);
                                            });
                                        })
                                    }else{
                                        callUpdateDone(error);
                                ...
                                retrieveParentData(inst.id, data, inst, callUpdateDone);
                            }
                        }
                    });
                }, data);
            } else {
                inst.constructor.create(inst, function (err, result) {
                    var callSaveDone = function (error, obj) {
                        saveDone.call(inst, function () {
                            callback(err || error, obj || inst);
                        });
                    };
                    if(err){
                        callSaveDone();
                    }else if(typeof inst.constructor.parentSchema !== 'undefined') {
                        backupData.id = inst.id;
                        inst.constructor.parentSchema.create(backupData, function (err, result) {
                            callSaveDone(err, result);
                        });
                    }else{
                        callSaveDone();
                    }
...
AbstractClass.prototype.toObject = function (onlySchema) {
    ...
    //TODO INHERITANCE. IF descommented create crashes. 
    if(typeof this.constructor.parentSchema !== 'undefined'){
        this.constructor.parentSchema.forEachProperty(function (attr) {
            if (self[attr] instanceof List) {
                data[attr] = self[attr].toObject();
            } else if (self.__data.hasOwnProperty(attr)) {
                data[attr] = self[attr] || self.__data[attr];
            } else {
                data[attr] = null;
            }
        });
...
AbstractClass.prototype.destroy = function (cb) {
    ...
    this.trigger('destroy', function (destroyed) {
        this._adapter().destroy(this.constructor.modelName, this.id, function (err) {
            var callDestroyed = function (error) {
                destroyed(function () {
                    if(cb) cb(err || error);
                });
            }
            ...
            if(err){
                ...
                if(typeof this.constructor.parentSchema !== 'undefined') {
                    findParentInstance.call(this.constructor, this.id, function (error, parentInstance) {
                        parentInstance.destroy(function (error, result) {
                            callDestroyed(error);    
                        })
                    });
                }else{
 ...
 * @param {Function} callback - callback called with (err, instance)
 */
AbstractClass.prototype.updateAttributes = function updateAttributes(data, cb, otherModel) {
    if (stillConnecting(this.constructor.schema, this, arguments)) return;

    var inst = this;
    var model = otherModel || this.constructor.modelName;
        ...
        inst.trigger('save', function (saveDone) {
            inst.trigger('update', function (done) {
                ...
                inst._adapter().updateAttributes(model, inst.id, inst.constructor._forDB(data), function (err) {
                    var callDone = function (error, obj) {
                        done.call(inst, function () {
                            saveDone.call(inst, function () {
                                cb(err || error, obj || inst);
                            });
                        });
                    };
                    ...
                    if (!err) {
                        ...
                        if(typeof otherModel ==='undefined' && typeof inst.constructor.parentSchema !== 'undefined') {
                            inst.constructor.parentSchema.find(inst.id, function (error, parent) {
                                parent.updateAttributes(data, function (error, parent) {
                                    if (error) {
                                        callDone(error, parent);
                                    }else{
                                        Object.keys(data).forEach(function (key) {
                                            inst.__dataWas[key] = inst.__data[key];
                                        });
                            ...
                            //inst.updateAttributes(data, function (error, inst) { 
                            //inst.updateAttributes.call(inst.__proto__.__proto__, data, function (error, inst) {
