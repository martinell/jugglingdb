 ...
 * ```
 */
Schema.prototype.define = function defineClass(className, properties, settings, parentSchema) {
    var schema = this;
    var args = slice.call(arguments);

    if (!className) throw new Error('Class name required');
    if (args.length == 1) properties = {}, parentSchema = {}, args.push(properties);
    if (args.length == 2) settings   = {}, args.push(settings);

    /*if(typeof parentSchema !== 'undefined' && parentSchema!==null){
        _.extend(properties, schema.definitions[parentSchema.modelName].properties);
    }*/
    ...
    hiddenProperty(NewClass, 'parentSchema', parentSchema);
